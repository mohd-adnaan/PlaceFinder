...
final class SemanticRouteNavigator: ObservableObject {
    ...
    func snapToNearestGraphPose(arPosition: simd_float3?, imuState: IMUState) {
        ...
        if let index = routeSteps.firstIndex(where: { $0.edge.id == edgeMatch.edge.id }) {
            currentStepIndex = index
            segmentProgressMeters = edgeMatch.alongTrackMeters
            segmentRemainingMeters = max(0, edgeMatch.edge.distanceMeters - edgeMatch.alongTrackMeters)
            phase = .navigating
            recoveryReason = nil
            updateInstruction(forceSpeech: true)
        } else {
            currentInstruction = "Nearest graph edge is \(edgeMatch.edge.spokenContext ?? edgeMatch.edge.id)."
        }
        rebuildRAGContext()
    }
    ...
    private func updateInstruction(forceSpeech: Bool) {
        ...
        let context = step.edge.spokenContext ?? "toward \(step.to.name)"
    ...
    private func nearbyLandmarkNames() -> [String] {
        ...
        return map.landmarks
            ...
            .map { landmark in
                if let context = landmark.context, !context.isEmpty {
                    return "\(landmark.name): \(context)"
                }
    ...
    private func nextLandmarkPhrase(on step: SemanticRouteStep, after progressMeters: Double) -> String? {
        ...
        return map.landmarks.compactMap { landmark -> (ahead: Double, phrase: String)? in
            ...
            return (ahead, "\(landmark.name) \(Self.sidePhrase(side)) in \(Self.formatMeters(ahead))")
        }
        .min { $0.ahead < $1.ahead }?
        .phrase
    }
    ...
    private func upsertMap(_ map: SemanticRouteMap, persist: Bool) {
        maps.removeAll { $0.id == map.id || Self.matches($0.name, map.name) }
    ...
    private static func appendedContext(_ existing: String?, _ addition: String) -> String {
        ...
        if existing.localizedCaseInsensitiveContains(addition) { return existing }
